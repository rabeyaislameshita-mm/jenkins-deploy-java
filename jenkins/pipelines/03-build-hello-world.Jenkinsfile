pipeline {
      ...
      steps {
        dir("${env.REPO_PATH}") {
          sh '''
            ...
            cp sbom-hello-world.cdx.json ${EVIDENCE_DIR}/sbom-${IMAGE_NAME}.cdx.json
          '''
        }
      }
    }
  ...
  post {
    always {
      dir("${env.REPO_PATH}") {
        archiveArtifacts artifacts: 'sbom-hello-world.cdx.json,evidence/*', onlyIfSuccessful: false
      }
    }
  }
}